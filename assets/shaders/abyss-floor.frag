...
void main(){
    ...
    btime *= texture2D(u_noise, vec2(sin(coords.x), cos(coords.y))).r;
    btime *= sin(btime);

    noise1 *= 100.0;
    noise2 *= 100.0;
    noise3 *= 100.0;
    float rel = dis(vec2(noise1, noise2), origin);
    vec2 com = vec2(cos(noise3) * rel, sin(noise3) * rel);
    float cr = pow(com.x, com.y);
    noise3 *= cr;
    color.rgb += 0.3 * smoothstep(1.0, 0.0, sin(dis(coords * coords, vec2(cos(noise3) * rel + cr * cos(btime), sin(noise3) * rel - cr * sin(btime + 114514.0))) / 100.0));
    btime *= color.r;
    btime /= texture2D(u_noise, vec2(sin(coords.x), cos(coords.y))).r + 0.001;
    color.r = 1.0 - 0.5 * (sin(btime) + 1.0) * pow(color.r, 3.0);
    color.g = 1.0 - 0.5 * (sin(btime + 1.57) + 1.0) * pow(color.g, 3.0);
    color.b = 1.0 - 0.5 * (cos(btime) + 1.0) * pow(color.b, 3.0);
    color.r = 1.0 - pow(color.r, 3.0);
    color.g = 1.0 - pow(color.g, 3.0);
    color.b = 1.0 - pow(color.b, 3.0);
    color.r = 1.0 - pow(color.r, 5.0);
    color.g = 1.0 - pow(color.g, 5.0);
    color.b = 1.0 - pow(color.b, 5.0);
    color.r = 1.0 - pow(color.r, 7.0);
    color.g = 1.0 - pow(color.g, 7.0);
    color.b = 1.0 - pow(color.b, 7.0);
